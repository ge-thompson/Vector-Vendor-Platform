...
{
    /// <summary>
    /// REFERENCE CATALOG — EDI 214 X12 status and reason codes for FourKites dispatcher
    /// EventUpdate.StatusCode / StatusReasonCode and StopUpdate fields.
    ///
    /// This is reference data, NOT active mapping logic. The live translation
    /// (framework LoadStatusType -> EDI 214 code, with DB overrides and pass-through)
    /// lives in <see cref="LoadStatusMapper"/>. Use the catalog here when refining that
    /// mapper after production data shows what FK actually accepts (open item O-002).
    ///
    /// Salvaged verbatim from the first-generation FourKitesIntegration.Core tree
    /// (now retired to _attic) so the negotiated EDI 214 vocabulary and the
    /// human-readable descriptions aren't lost. FourKites does not enforce a controlled
    /// vocabulary — these are the de facto industry-standard codes already used in
    /// Vector's EDI 214 messages, so they migrate to the API unchanged.
    /// </summary>
    public static class Edi214Reference
    {
        /// <summary>
